...
class M_transaksi extends CI_Model {
	 public function simpan_cart_db()
	 {
	 	for($i=0; $i<count($this->cart->contents()); $i++){
				...
				$stok = $this->db->where('id_minum', $this->input->post('id_minum')[$i])
								 ->get('minum')
								 ->row()
								 ->stok;
				...
				$this->db->where('id_minum', $this->input->post('id_minum')[$i])
						 ->update('minum', $updatestok);
		}
	 	...
	 	for ($i=0;$i<count($this->input->post('rowid')) ;$i++) { 
	 		...
	 		'id_minum' =>$this->input->post('id_minum')[$i],
	 ...
	 {
	 	return $this->db->where('id_transaksi', $id_transaksi)
	 					->join('minum','minum.id_minum=nota.id_minum')
	...
	public function cek($id_minum){

		$cek_stok = $this->db->where('id_minum', $id_minum)->get('minum')->row()->stok;
